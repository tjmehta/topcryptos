...
def evaluate(index, signal, holding, policy, atr, resistance):
    ...
    if policy == 'ResistanceSMAATR' and resistance is None:
        target = None
    if policy != 'FixedH':
...
def main():
    ...
    source_bytes = (SOURCE/'cohorts.json.gz').read_bytes()
    source_manifest = json.loads((SOURCE/'summary.json').read_text())
    if hashlib.sha256(source_bytes).hexdigest() != source_manifest['ledger_sha256']:
        raise SystemExit('Selection ledger hash does not match its frozen manifest')
    source_rows = json.loads(gzip.decompress(source_bytes))
