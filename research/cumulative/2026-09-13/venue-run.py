...
def cumulative(prices,times):
    ...
    contributions=[(b-a)*(x+y)/2 for a,b,x,y in zip(times,times[1:],ys,ys[1:])]
    return rounded_area(contributions)/(times[-1]-times[0])


def rounded_area(contributions):
    area=math.fsum(contributions)
    magnitude=math.fsum(abs(x) for x in contributions)
    return 0. if abs(area)<=32*2**-52*magnitude else area
...
def selftest():
    ...
    assert cumulative([100,200,100,50,100],[0,1,2,3,4])==0
    assert cumulative([1,2,1,.5,1],[0,1,2,3,4])==0
...
def main():
    ...
    for i,f in enumerate(source['formations']):
        ...
        for identity in f['eligible']:
            ...
            cum[identity]=rounded_area([.5*(a+b) for a,b in zip(ys,ys[1:])])/(view-1)
