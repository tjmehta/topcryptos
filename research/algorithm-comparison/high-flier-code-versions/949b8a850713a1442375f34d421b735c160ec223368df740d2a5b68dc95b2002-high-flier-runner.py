...
def bar_is_complete(bar: Bar) -> bool:
    return bar.bar_status in {"valid", "complete"}


def bar_open_is_executable(bar: Bar) -> bool:
    """Allow valid or partial opens only with evidence of actual trading activity."""
    return (
        ...
        and (bar_is_complete(bar) or "partial_interval" in bar.bar_status.split("|"))
    )
    ...
    for key, raw_bars in sorted(grouped_raw.items()):
        ...
        for day in sorted(by_date):
            ...
            if len(same_day) == 1:
                ...
                first = same_day[0]
                flags = sorted(
                    set(first.bar_status.split("|")) | {"duplicate_date"}
                )
                bars.append(dataclasses.replace(first, bar_status="|".join(flags)))
...
def evaluate_exit(
    ...
    if early:
        ...
        if any(
            bar is not None and bar.high >= 1.10 * exit_bar.open for bar in post_exit
        ):
            premature = True
        elif any(bar is None for bar in post_exit):
            premature = None
...
def moving_block_ratio_bootstrap(
    ...
    return {
        ...
        "available_nonoverlapping_calendar_blocks": len(calendar) // block_days,
...
def read_panel_csv(
    ...
    for row_number, row in enumerate(reader, start=2):
        ...
        strict_numeric = row["bar_status"] == "complete"
        ...
        rows.append(
            Bar(
                ...
                scheduled_midnight_open=(
                    (row["symbol"], day) not in (non_midnight_open_dates or set())
                ),
            )
        )
...
def exit_metrics(exit_rows: Sequence[Mapping[str, Any]], rule: str) -> dict[str, Any]:
    ...
    return {
        ...
        "premature_exit_10pct": {
            "true": sum(row["premature_10pct_status"] is True for row in known),
            "false": sum(row["premature_10pct_status"] is False for row in known),
            "unknown": sum(row["premature_10pct_status"] is None for row in known),
        },
        "fee_sensitivity": {
...
def _paired_discovery_bootstrap(
    ...
    result["contrast"] = "Breakout20 minus Return7 known MFE7>=20% hits per ten slots"
...
def paired_exit_metrics(exit_rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for episode_id in episode_ids:
        ...
        for scenario, fallback in (("zero_return", 0.0), ("total_loss", -1.0)):
            scenario_differences[scenario].append(
                (float(trail_net) if trail_net is not None else fallback)
                - (float(fixed_net) if fixed_net is not None else fallback)
            )
    return {
    ...
    for episode_id in episode_ids:
        ...
        for scenario, fallback in (("zero_return", 0.0), ("total_loss", -1.0)):
            scenarios[scenario][entry_date].append(
                (float(trail_value) if trail_value is not None else fallback)
                - (float(fixed_value) if fixed_value is not None else fallback)
            )
...
def build_summary(
    ...
    for block in partitions:
        ...
        block_episodes = _filter_rows(episodes, "entry_date", start, end)
        block_exits = _filter_rows(exit_rows, "entry_date", start, end)
        blocks[name] = {
