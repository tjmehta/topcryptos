...
def panel_csv_bytes(rows: list[dict[str, str]]) -> bytes:
    columns = (
        "cohort_year",
        "reporting_block",
        "instrument_id",
...
def schema_row(
    *,
    ...
    return {
        ...
        "symbol": "AAAUSDT",
        "identity_segment_id": instrument_id,
        "date": date,
        "open_time_us": "1577836800000000",
        "close_time_us": "1577923199999000",
        "open_time_raw": "1577836800000",
        "close_time_raw": "1577923199999",
        "timestamp_unit": "ms",
        "open": "100",
        "high": "101",
        "low": "99",
        ...
        "taker_buy_quote_volume": "1000000",
        "source_archive": "AAAUSDT-1d-2020-01.zip",
        "source_row": "1",
...
class DataSchemaTests(unittest.TestCase):
    def test_actual_status_strings_control_history_path_and_execution(self) -> None:
        rows = [
            schema_row(
                instrument_id="complete-detail",
                date="2020-01-01",
                bar_status="complete|zero_trade_count|zero_quote_volume",
                trade_count="0",
                base_volume="0",
                quote_volume="0",
            ),
            schema_row(
                ...
                date="2020-01-01",
                bar_status="partial_terminal|partial_interval",
            ),
            schema_row(
                instrument_id="late",
                date="2020-01-01",
                bar_status="complete|non_midnight_first_trade",
            ),
            schema_row(
                instrument_id="invalid",
                date="2020-01-01",
                bar_status="invalid|invalid_open",
            ),
        ]
        panel = runner.read_panel_csv(
            panel_csv_bytes(rows), {(2020, "AAAUSDT")}, "gzip"
        )
        complete = panel["2020:complete-detail"][0]
        partial = panel["2020:partial"][0]
        late = panel["2020:late"][0]
        invalid = panel["2020:invalid"][0]
        self.assertTrue(runner.bar_is_complete(complete))
        self.assertFalse(runner.bar_open_is_executable(complete))
        self.assertFalse(runner.bar_is_complete(partial))
        self.assertTrue(runner.bar_open_is_executable(partial))
        self.assertTrue(runner.bar_is_complete(late))
        self.assertFalse(late.scheduled_midnight_open)
        self.assertFalse(runner.bar_open_is_executable(late))
        self.assertFalse(runner.bar_is_complete(invalid))

    def test_complete_detail_rows_use_strict_numeric_parsing(self) -> None:
        row = schema_row(
            instrument_id="strict",
            date="2020-01-01",
            bar_status="complete|zero_trade_count",
        )
        row["open"] = "not-a-number"
        with self.assertRaises(runner.ProtocolError):
            runner.read_panel_csv(
                panel_csv_bytes([row]), {(2020, "AAAUSDT")}, "gzip"
            )

    def test_duplicate_and_invalid_rows_are_retained_as_unusable(self) -> None:
        first = schema_row(
        ...
        second["source_row"] = "2"
        panel = runner.read_panel_csv(
            panel_csv_bytes([first, second]), {(2020, "AAAUSDT")}, "gzip"
        )
        retained = panel["2020:duplicate"]
        self.assertEqual(len(retained), 1)
        self.assertEqual(runner.bar_status_class(retained[0]), "invalid")
        self.assertIn("duplicate_date", runner.bar_status_flags(retained[0]))
...
class EligibilityAndScoringTests(unittest.TestCase):
    def test_requires_sixty_complete_consecutive_bars_and_lagged_volume(self) -> None:
        ...
        replace_bar(bars, 20, bar_status="partial_terminal|partial_interval")
...
class ForwardGradingTests(unittest.TestCase):
    ...
    def test_partial_open_requires_positive_activity_but_cannot_grade_path(self) -> None:
        ...
        signal_index = 60
        entry_index = 62
        replace_bar(bars, entry_index, bar_status="partial_terminal|partial_interval", trade_count=2)
...
class EpisodeAndFeeTests(unittest.TestCase):
    ...
    def test_missing_zero_gross_scenario_pays_same_fees_before_pairing(self) -> None:
        zero_net = runner.net_return_from_gross(0.0, 50)
        rows = [
            {
                "episode_id": 1,
                "rule": "Trail15",
            ...
            },
            {
                ...
                "rule": "Fixed30",
                "gross_return": 0.0,
                "net_returns": {"50": zero_net},
            },
        ]
        metrics = runner.paired_exit_metrics(rows)
        self.assertAlmostEqual(
            metrics["net_50bps_difference_zero_missing_scenario"]["mean"],
            0.0,
        )


class AttributionTests(unittest.TestCase):
    def test_episode_and_exit_blocks_follow_signal_date_not_entry_date(self) -> None:
        grade = {
            "path_status": "known",
            "endpoint_status": "known",
            "absolute_hit": False,
            "mfe": 0.0,
            "mae": 0.0,
            "endpoint_return": 0.0,
        ...
        episode = {
            ...
            "grades": {"7": dict(grade), "30": dict(grade)},
        }
        exit_rows = []
        for rule in runner.EXIT_RULES:
            exit_rows.append(
                {
                    "episode_id": 1,
                    "rule": rule,
        ...
        manifest = {
            "study": {"signal_start": "2020-01-01", "signal_end": "2025-12-31"},
            "blocks": [
                {"name": "development", "start": "2020-01-01", "end": "2022-12-31"},
                {"name": "evaluation", "start": "2023-01-01", "end": "2025-12-31"},
            ],
        }
        summary = runner.build_summary(
            ...
            [],
            [],
            {method: [] for method in runner.METHODS},
            [episode],
            exit_rows,
        )
        self.assertEqual(
            summary["blocks"]["development"]["episodes"]["Return7"]["7"]["episodes"],
            1,
        )
        self.assertEqual(
            summary["blocks"]["evaluation"]["episodes"]["Return7"]["7"]["episodes"],
            0,
        )
        self.assertEqual(
            summary["blocks"]["development"]["exits"]["Fixed30"]["entries"],
            1,
        )
        self.assertEqual(
            summary["blocks"]["evaluation"]["exits"]["Fixed30"]["entries"],
            0,
        )

    def test_exit_bootstrap_calendar_ends_on_latest_included_entry(self) -> None:
        rows = []
        for episode_id, entry_date in ((1, "2020-01-01"), (2, "2026-01-02")):
            for rule in ("Trail15", "Fixed30"):
                rows.append(
                    {
                        "episode_id": episode_id,
                        "rule": rule,
                        "entry_date": entry_date,
                        "net_returns": {"50": 0.0 if episode_id == 1 else None},
                    }
                )
        result = runner._paired_exit_bootstraps(rows)["complete_pairs_conditional"]
        self.assertEqual(result["calendar_end"], "2026-01-02")
...
class ExitTests(unittest.TestCase):
    def test_trail_trigger_fills_two_days_later_at_open_not_peak(self) -> None:
        ...
        self.assertTrue(outcome.premature_10pct_status)

    def test_premature_exit_requires_complete_post_exit_path(self) -> None:
        bars = make_bars(130)
        signal_index = 60
        entry = 62
        replace_bar(bars, entry, open=100.0, high=110.0, low=99.0, close=110.0)
        replace_bar(bars, entry + 1, open=110.0, high=120.0, low=90.0, close=90.0)
        replace_bar(bars, entry + 3, open=95.0, high=96.0, low=94.0, close=95.0)
        replace_bar(
            bars,
            entry + 10,
            high=200.0,
            bar_status="invalid|duplicate_date",
        )
        trail = runner.evaluate_exit("Trail15", bars, bars[signal_index].date)
        fixed = runner.evaluate_exit("Fixed30", bars, bars[signal_index].date)
        self.assertTrue(trail.early_exit)
        self.assertIsNone(trail.premature_10pct_status)
        self.assertFalse(fixed.early_exit)
        self.assertIsNone(fixed.premature_10pct_status)
...
class BootstrapTests(unittest.TestCase):
    ...
    def test_discovery_bootstrap_exposes_fraction_and_per_ten_units(self) -> None:
        start = dt.date(2020, 1, 1)
        decision_dates = [start + dt.timedelta(days=index) for index in range(60)]
        alerts = [
            {
                "method": "Breakout20",
                "signal_date": start.isoformat(),
                "grades": {"7": {"absolute_hit": True}},
            }
        ]
        result = runner._paired_discovery_bootstrap(alerts, decision_dates)
        self.assertEqual(result["status"], "ok")
        self.assertAlmostEqual(result["estimate_fraction"], 1.0 / 600.0)
        self.assertAlmostEqual(result["estimate_hits_per_ten_slots"], 1.0 / 60.0)
        self.assertEqual(
            result["ci95_hits_per_ten_slots"],
            [10.0 * value for value in result["ci95_fraction"]],
        )
