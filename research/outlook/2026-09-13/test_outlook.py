...
class OutlookTests(unittest.TestCase):
    ...
    def test_transitive_helper_hash_mismatch_prevents_import(self):
        original_read=Path.read_bytes
        for relative in ['research/holding-horizons/run.py',
                         'research/algorithm-comparison/high-flier-runner.py']:
            target=m.ROOT/relative
            def changed_bytes(path):
                raw=original_read(path)
                return raw+b'\n# mocked mismatch\n' if path==target else raw
            m.helpers.cache_clear()
            try:
                with mock.patch.object(Path,'read_bytes',changed_bytes), mock.patch.object(m,'read_module') as imported:
                    with self.assertRaisesRegex(ValueError,'Frozen transitive helper hash mismatch'):
                        m.helpers()
                    imported.assert_not_called()
            finally:
                m.helpers.cache_clear()
        # Real frozen files were never written and still load after the mocks end.
        self.assertEqual(len(m.helpers()),3)
